...
public class routinesTS extends TinyLanguageSIIBaseListener {
    ...
    private static final String ERR_INSTANTIATION =" variable non instatiated : ";
                    ...
                    if (ctx.exp(0).ID()!= null){        //L'expression de gauche est un id
                        ...
                        if(!dec1){                      //L'id n'a pas été déclaré
                            ...
                            if (!init1)                 //L'id n'a pas été initialisé
                                errors.add("Semantic error at line "+ctx.getStart().getLine()+":"+ctx.getStart().getCharPositionInLine()+ ERR_INSTANTIATION+ ctx.exp(0).ID().toString());
                        }
                    }
                    ...
                    if (ctx.exp(1).ID()!= null) {           //L'expression de droite est un id
                        ...
                        if (!dec2) {                        //L'id n'a pas été déclaré
                            ...
                            if (!init2)                     //L'id n'a pas été initialisé
                                errors.add("Semantic error at line "+ctx.getStart().getLine()+":"+ctx.getStart().getCharPositionInLine()+ ERR_INSTANTIATION+ ctx.exp(1).ID().toString());
                        }
                    }
